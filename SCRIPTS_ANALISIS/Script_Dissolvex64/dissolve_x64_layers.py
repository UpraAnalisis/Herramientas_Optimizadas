...
scriptAuxiliar="dissolve_x64_aux_layers.py"
...
multi_part=arcpy.GetParameterAsText(2)
unsplit_lines=arcpy.GetParameterAsText(3)
...
##    arcpy.AddMessage(verPythonDir)
    ...
    comando=r"start %s %s %s %s %s %s %s %s"%(verPython,script,infea,campo,capa_salida,gdb_salida,multi_part,unsplit_lines)
    ...
    arcpy.AddMessage(capa_salida)
    layer_salida=arcpy.MakeFeatureLayer_management(r"%s"%(output),capa_salida)
