...
if capa_exclusion!="":
    capa_exclusion=arcpy.Describe(capa_exclusion).catalogPath
else:
    capa_exclusion="----"

consulta = arcpy.GetParameterAsText(6) # Expresión que actua como un criterio, que aplicará el eliminate sobre los fetures que cumplan con lo especificado en dicha expresión.
nombreSalida=arcpy.GetParameterAsText(7) # captura el nombre de salida del feature class resultante del proceso

scriptAuxiliar="Eliminate_Polygons_MultiExtent_centroid_v1.1.py" # script auxiliar que ejecuta el prceso de eliminate
verPythonfinal=r"C:\Python27\ArcGISx6410.5\python.exe" # ruta del ejecutable de python
...
if __name__ == '__main__':
    ...
    if len(no_existen)==0: # si el arreglo de no existen no tiene ningún elemento procesa la unificación
        ...
        arcpy.SetParameter(8, layer_salida)
