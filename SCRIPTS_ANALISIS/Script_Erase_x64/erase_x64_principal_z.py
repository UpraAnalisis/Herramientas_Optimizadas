...
scriptAuxiliar="erasex64_aux_z.py"
verPythonfinal=verPython64

infea=r"%s"%(arcpy.GetParameterAsText(0))
infea=arcpy.Describe(infea).catalogPath
feaUpdate=r"%s"%(arcpy.GetParameterAsText(1))
feaUpdate=arcpy.Describe(feaUpdate).catalogPath
...
##    arcpy.AddMessage(verPython)
    ...
    comando=r"start %s %s %s %s %s"%(verPython,script,infea,feaUpdate,capa_salida)
...
##    arcpy.SetParameter(4, layer_salida)
