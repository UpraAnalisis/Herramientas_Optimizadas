...
# ---------------------------------------------------------------------
#=====================Librerias==============================#
import arcpy, os ,inspect
# ------------------------------------------------------------

#=====================Parámetros==============================#
layer = arcpy.GetParameterAsText(0) # FeatureLayer sobre el cual se construirán los intervalos de clases geométricos y se asignarán las clases.
...
clases =  arcpy.GetParameterAsText(2).split(";") # En este campo se deben escribir los nombres que recibirán los intervalos de clases. Ejemplo: Alta;Media;Baja
campo_calculate = arcpy.GetParameterAsText(3) # En este campo se asignarán los nombres de los intrevalos de clases de acuerdo con el valor del campo de valores
...
def asignasimbologia(layer,campo,num_clases,clases,ruta): # asigna la simbología y devuelve los intervalos
    ...
    sourceLayer = arcpy.mapping.Layer(ruta+"\\layers\\"+"intervalos_geometricos_alta_media_baja.lyr") # captura el layer de ejemplo que define la simbología
...
def asignacategoria(capa,campo_clasificacion,intervalos,clases,campo_calculate): # asigna el nombre de clase acorde con los intervalos
    ...
    with arcpy.da.UpdateCursor(capa,[campo_clasificacion,campo_calculate]) as cursor:# actualiaz el campo del layer
        for fila in cursor:
...
if __name__ == '__main__':
    ...
    rangos = asignasimbologia(layer,campo_clasificacion,num_clases,clases,ruta) # asigna la simbología y devuelve los intervalos
    ...
    arcpy.SetParameter(4,arcpy.Describe(layer).catalogpath) # fija el parámetro de salida que es un feature class
